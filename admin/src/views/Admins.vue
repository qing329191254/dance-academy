<template>
  <div class="admins-page page-card">
    <div class="toolbar">
      <div v-if="!isMobile" class="hint">超级管理员可创建超级管理员或管理员，并为管理员分配可管理的校区。</div>
      <el-button type="primary" class="toolbar-add" @click="edit()">新增管理员</el-button>
    </div>

    <div v-if="isMobile" class="mobile-feed">
      <div v-for="row in list" :key="row.id" class="mobile-feed-item">
        <div class="mobile-feed-head">
          <span class="mobile-feed-title">{{ row.name || row.username || '—' }}</span>
          <span class="mobile-feed-status">{{ roleLabel(row.role) }}</span>
        </div>
        <div class="mobile-feed-main">{{ row.username || '—' }}</div>
        <div class="mobile-feed-meta mobile-summary">{{ campusScopeLabel(row) }}</div>
        <div class="table-actions">
          <el-button link type="primary" @click="edit(row)">编辑</el-button>
          <el-button link type="danger" :disabled="row.id === auth.profile?.id" @click="remove(row)">删除</el-button>
        </div>
      </div>
      <div v-if="!list.length" class="mobile-feed-empty">暂无管理员</div>
    </div>
...
function campusScopeLabel(row) {
  if (row.superAdmin) return '全部校区'
  return (row.campusIds || []).map(campusName).join('、') || '—'
}
